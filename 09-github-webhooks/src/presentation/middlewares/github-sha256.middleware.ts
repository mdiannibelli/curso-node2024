...
export class GithubSha256Middleware {
    ...
    static async verifyGithubSignature(req: Request, res: Response, next: NextFunction) {
        const xHubSignature = `${req.headers['x-hub-signature-256']}`; // read Github docummentation => https://github.com/mdiannibelli/curso-node2024/settings/hooks/525384908
        ...
        if (isValid) {
            ...
            res.status(401).json({ error: 'Invalud signature' });
        }
    }
